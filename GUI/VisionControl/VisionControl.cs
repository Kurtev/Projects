   ...
   
{
  ...
  {
//SQL C# Conection Method
      void FillCombo()
      {
          string constring = "datasource=localhost;port=3307;username=root;password=123456";
          string Query = "select * from test.prod_bag;";
          ...
          try
          {
              ...
              while (myReader.Read())
              {
                  string sName = myReader.GetString("Bag_code");
    ...
    private void btnRunCont_Click(object sender, System.EventArgs e)
    {
         ...
         
      {
        // Stop continuous
        mJM.Stop();
        textBox24.Text = DateTime.Now.ToString();
      ...
      else
      {
        ...
        {
          mJM.RunContinuous();
          textBox23.Text = DateTime.Now.ToString();
        }
        catch (Cognex.VisionPro.Exceptions.CogNotStoppedException)
        {
          // do nothing in this case - we're already running
    ...
    private void button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            myDataAdapter.SelectCommand = new MySqlCommand("select * test.prod_bag;", myConn);
    ...
    {
        string constring = "datasource=localhost;port=3307;username=root;password=123456";
        string Query = "select * from test.prod_bag where Bag_code='" + comboBox1.Text + "'  ;";
        MySqlConnection conDataBase = new MySqlConnection(constring);
        MySqlCommand cmdDataBase = new MySqlCommand(Query, conDataBase);
        MySqlDataReader myReader;
        try
        {
            ...
            while (myReader.Read())
            {
                string sId= myReader.GetInt32("Bag_id").ToString();
                string sBagCap = myReader.GetString("Bag_capacity");
                string sFoilType = myReader.GetString("Outter_foil_type");
                string sInsertType = myReader.GetString("Insert_type");
                string sTapType = myReader.GetString("Tap_type");
                string sTapLocation = myReader.GetString("Tap_location");
                string sTapDirection = myReader.GetString("Tap_direction");
                string sTapDepth = myReader.GetString("Tap_depth");
                string sTapWeght = myReader.GetString("Bag_weight");
                string sBagLenght = myReader.GetString("Bag_lenght");
                string sBagWidth = myReader.GetString("Bag_width");

                textBox1.Text = sBagCap;
                textBox2.Text = sFoilType;
                textBox3.Text = sInsertType;
                textBox4.Text = sTapType;
                textBox5.Text = sTapLocation;
                textBox6.Text = sTapDirection;
                textBox7.Text = sTapDepth;
                textBox8.Text = sTapWeght;
                textBox9.Text = sBagLenght;
                textBox10.Text = sBagWidth;
    ...
    private void textBox_JobN_TotalIterations_TextChanged_1(object sender, EventArgs e)
    {
        //Use StreamWriter class.

        StreamWriter sw = new StreamWriter("C:\\JobResults.csv");


        //Use writeline methode to write the text
        sw.WriteLine("Last Job Tap/Gland Results");
        sw.WriteLine("Total Iterations:");
        sw.WriteLine(textBox_JobN_TotalIterations.Text);
        sw.WriteLine("Accept:");
        sw.WriteLine(textBox_JobN_TotalAccept.Text);
        sw.WriteLine("Reject:");
        sw.WriteLine(textBox_JobN_TotalReject.Text);
        sw.WriteLine("Total Accept Percent :");
        sw.WriteLine(textBox_JobN_TotalAccept_Percent.Text);
        String fileCreationTime = DateTime.Now.ToString();
        sw.WriteLine("Start Events :");
        sw.WriteLine(textBox23.Text);
        sw.WriteLine("Stop Events :");
        sw.WriteLine(textBox24.Text);
        sw.WriteLine("File Creation Time :");
        sw.Write(fileCreationTime);
        sw.WriteLine(" ");
        sw.Write("Version: 4.02");
        sw.WriteLine(" ");
        Text = Text + "\r\n";
